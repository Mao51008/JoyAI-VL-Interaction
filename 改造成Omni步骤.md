...
- [x] A3 接入流式 ASR 后，已验证服务中断会产生 `asr_error`，重启后可恢复转写。
- [ ] 音频时间戳与服务器时间轴没有明显漂移。
...
- [x] 能回放一次会话，并按时间顺序重建视频、声音、文本和模型动作。
...
- [x] 通过独立快照决策引擎，在没有用户提问时仍持续执行门控判断。
- [x] 将 ASR partial、stable、final 和音频事件注入模型上下文。
- [x] 把动作协议扩展为：
  - `</silence>`
  - `</response> ...`
  - `</delegate> ...`
  - `</interrupt>`（停止当前语音）
- [x] 增加主动发言冷却时间和内容去重，避免每秒重复报警。
- [x] 增加事件优先级，例如生命安全事件可以绕过普通冷却。
- [x] 把“是否需要发言”和“具体说什么”拆成门控与响应模型两个逻辑阶段。
- [x] 决策记录快照序号/上下文指纹、动作、可用置信度、延迟和触发原因，并写回统一时间线。

验收：

- [x] 假模型测试中，没有人讲话时听到火警等危险声音能够主动输出。
- [x] 假模型测试中，普通稳定场景保持沉默。
- [x] 假模型测试中，同一持续事件不会每秒重复完全相同的警告。
