...
- [√] A3 接入流式 ASR 后，已验证服务中断会产生 `asr_error`，重启后可恢复转写。
- [ ] 音频时间戳与服务器时间轴没有明显漂移。
...
- [√] 已实现默认每 400 ms 调度一次的滑动窗口 partial ASR，并接入真实 Qwen3-ASR-1.7B。
- [√] 已实现单调增长的 stable prefix，避免每次重写整句话。
- [√] 已增加语音活动比例、说话开始和说话结束事件。
- [√] 已增加 CLAP 零样本环境声音分类器原型。
- [√] 已定义可插拔环境声音检测接口，结果包含类别、置信度和起止时间；真实分类器待 A3.3 接入。
- [√] 事件保留原始置信度并设置最低阈值，供决策层作为不确定观测而非确定事实。

A3.1 本地框架已完成：连续音频环形缓冲可按时间窗读取，调度器不会重复识别
没有新增数据的窗口，WebUI 可显示 `speech_partial`/`speech_stable`/`speech_final`，
模拟测试不依赖 GPU 或真实 ASR 服务。

A3.2 已在服务器 GPU1 上通过真实验收：使用 Qwen 官方 `qwen-asr-serve`
（Qwen3-ASR-1.7B、vLLM 0.14.0、最大上下文 8192）提供
`/v1/audio/transcriptions`，WebUI 每 400 ms 从最近 6 秒滑动窗口请求转写。
...
- [√] 播放火警/爆炸合成音频时，即使 ASR 文本为空也会产生音频事件。
...
- [√] 所有时间使用同一服务器单调时钟，墙上时间只用于日志展示。
- [√] 按固定 tick（默认 1 秒）生成一次多模态快照。
- [√] 允许突发高优先级事件绕过普通 tick，立即生成一次决策快照。
...
- 普通中文语音：131 个 40 ms 包零丢失，产生 4 个语音时间轴事件和 8 个
  `tick` 快照，没有高优先级误触发。
- 火警流：207 个包零丢失，产生 29 个时间轴事件、12 个 `tick` 快照和
  5 个由 `audio_event` 立即触发的 `priority` 快照；ASR 与检测器均零错误。
- 自动化测试覆盖音频、视频、文本、模型动作、TTS 区间的统一排序和有界回放。
...
- [√] 通过独立快照决策引擎，在没有用户提问时仍持续执行门控判断。
- [√] 将 ASR partial、stable、final 和音频事件注入模型上下文。
- [√] 把动作协议扩展为：
  - `</silence>`
  - `</response> ...`
  - `</delegate> ...`
...
- [√] 增加事件优先级，例如生命安全事件可以绕过普通冷却。
- [√] 把“是否需要发言”和“具体说什么”拆成门控与响应模型两个逻辑阶段。
- [√] 决策记录快照序号/上下文指纹、动作、可用置信度、延迟和触发原因，并写回统一时间线。

验收：

- [√] 假模型测试中，没有人讲话时听到火警等危险声音能够主动输出。
- [√] 假模型测试中，普通稳定场景保持沉默。
- [√] 假模型测试中，同一持续事件不会每秒重复完全相同的警告。

A5 软件侧和无 GPU 假模型链路已完成：`SnapshotContextBuilder` 组织视频观察、环境声音、
ASR、用户问题、TTS 状态和最近动作；`RuleBasedDecisionGate` 与
`FakeResponseModel` 完成两阶段决策；`OmniDecisionEngine` 使用 latest-only 队列、
优先级快照、冷却和去重，并把结果回写时间线。在 `OMNI_DECISION_MODE=fake` 时，
WebUI 会为会话安装决策引擎并向浏览器发送 `omni_decision`。

以上仅代表本地无 GPU 软件验收。真实 JoyAI-VL 的多模态上下文效果、主动发言质量和延迟
仍需等待 GPU0 可用后验证，不能据此认定 A5 已完成真实模型验收。GPU1 的 ASR 稳定性测试
已被手动停止，已有数字仅为停止前阶段性统计，不是完整 30 分钟稳定性验收。
...
- [√] 软件链路中 TTS 播放不关闭独立运行的连续麦克风和视频输入。
- [√] 每次 TTS 生成分配 `generation_id`。
- [√] TTS 服务支持 `cancel(generation_id)`，并按会话维护活动生成注册表。
- [√] 浏览器收到 `</interrupt>` 决策后停止当前音源并清空尚未播放的音频 buffer。
- [ ] 用户插话或高优先级事件出现时，在 200–500 ms 内停止旧回复。
- [√] 浏览器回报已播放时长和近似文本前缀，新决策上下文优先使用被打断前已播放文本。
- [√] 区分并记录“停止浏览器播放”“停止 TTS 生成”“取消语言模型生成”三个动作。

验收：

- [√] 无 GPU 假模型测试中，系统说话时用户插话或出现危险声音会产生打断并取消旧生成。
- [ ] 中止后系统能根据新输入继续，而不是重头重复旧回答。

A6 软件侧打断链路已完成：浏览器和服务端共同维护独立于请求序号的
`generation_id`；服务端可从 TTS WebSocket、会话清理或 Omni 决策回调精确取消活动生成；
`interrupt` 同时请求清空浏览器播放队列、停止 TTS 流并取消当前 VLM 任务。浏览器在停止前
回报实际播放时长和按已接收音频比例估算的文本前缀，统一时间线会区分计划文本与已播放文本。

尚未完成 A6 的真实浏览器/真实设备验收：200–500 ms 打断延迟、外放条件下的插话，以及
中止后基于新输入自然续答仍需在里程碑 A 软件功能全部完成后统一测试。
...
- [√] 连续麦克风和按键 ASR 均开启浏览器 acoustic echo cancellation。
- [√] 保存当前 TTS 文本、`generation_id`、播放区间和浏览器实际播放进度作为回声参考。
- [√] 服务端增加基于规范化文本和序列匹配的 TTS/ASR 回声相似度检测。
- [√] TTS 播放及尾音期间，把高度相似的 ASR partial/stable/final 标记为 `likely_tts_echo` 并从决策触发中排除。
- [√] TTS 播放时不关闭麦克风；检测到与参考文本不同的新内容仍保留真正的 barge-in。
- [ ] 使用扬声器和耳机两种场景分别测试。

验收：

- [√] 无 GPU 假模型测试中，纯 TTS 回声保持沉默，不会触发新回答或打断。
- [ ] 戴耳机和外放时都能正常插话。

A7 无 GPU 软件侧已完成：`TTSEchoFilter` 使用当前或刚结束的 TTS 文本作为参考，
对 ASR 文本计算序列相似度和观测文本覆盖率。至少 4 个规范化字符且相似度不低于
0.82 时标记为 `likely_tts_echo`；短公共词不会直接被抑制，包含新增用户内容的文本仍可
触发打断。`speech_start` 本身不再立即打断，门控等待首个 ASR partial 后区分回声与真实
插话，默认调度下目标延迟约为 400 ms。

可通过 `TTS_ECHO_SIMILARITY_THRESHOLD`、`TTS_ECHO_MIN_TEXT_CHARS` 和
`TTS_ECHO_TAIL_SECONDS` 调整阈值、最短文本和播放结束后的尾音窗口。当前实现属于文本
参考抑制，不等同于声学回声消除；真实扬声器、房间混响和耳机条件仍需在里程碑 A 集中验收。

### A8. 里程碑 A 的测试集

- [√] 无 GPU 假事件测试：无人说话、普通办公室帧保持沉默。
- [√] 无 GPU 合成 `visual_event` 测试：火灾视觉事件、无声音时主动警告。
- [√] 无 GPU 合成音频事件测试：无视觉证据、只有火灾警报时主动警告。
- [√] 无 GPU 合成事件测试：视频烟雾和声音警报融合为一次高优先级决策。
- [√] 用户说话中途修改问题时，partial 保持沉默，只在 final 后响应。
- [√] 系统说话时用户插话会中止旧回复。
- [√] 系统说话时出现高优先级警报，会先中止普通回复并在后续快照报警。
- [√] 重叠说话元数据会保留 ASR 低置信度并明确提示内容可能不完整。
- [√] 虚拟时钟 30 分钟长流测试确认时间线、快照和决策记录保持有界。

A8 无 GPU 软件测试集已完成，集中在 `test_a8_scenarios.py`，并与 A2–A7 的单元和集成
测试共同运行。假门控新增 `visual_event`/`observation` 危险标签，支持纯视觉火焰/烟雾
以及视觉烟雾 + 火警声音的融合触发。30 分钟测试使用 1800 个虚拟 1 秒 tick，不等待真实
时间，验证时间线淘汰、快照历史、决策记录上限和安静场景零响应模型调用。

这些勾选仅表示合成事件和虚拟时钟的软件验收。真实火灾视频的视觉理解依赖 JoyAI-VL，
真实黑屏音频依赖 Qwen3-ASR/CLAP，打断延迟、自声率、误报、漏报、实时系数和 GPU 显存
仍需在里程碑 A 集中真实设备/GPU 验收后记录。

#### GPU 恢复后的第一优先级测试清单

2026-07-30 检查时服务器 4 张 GPU 均被占用，因此本轮没有启动 JoyAI-VL、ASR、TTS
或任何 GPU 测试进程。不要重启此前被用户手动停止的 GPU1 ASR 稳定性任务，也不要把
停止前统计写成完整 30 分钟验收。

GPU 再次可用并确认获得对应卡的使用授权后，按以下顺序执行：

1. [ ] 用 `nvidia-smi` 确认获授权 GPU、空闲显存和现存进程；不得结束其他用户进程。
2. [ ] 同步 `feature/continuous-omni`，先运行 WebUI 全量无 GPU 测试作为回归基线。
3. [ ] 在 GPU0 单独启动 JoyAI-VL，验证普通办公室、真实火灾视频和真实烟雾视频。
4. [ ] 释放 GPU0 后单独启动 Qwen3-ASR，验证普通语音、说话中途改口、服务恢复和时间戳。
5. [ ] 使用 CPU CLAP 配合真实火警/爆炸音频，记录置信度、误报、漏报和触发延迟。
6. [ ] 释放 GPU0 后单独启动 Qwen3-TTS，验证 `generation_id` 取消和取消后的残留音频。
7. [ ] 获得多卡授权后再联合启动 VLM、ASR、TTS；CLAP 继续使用 CPU。
8. [ ] 使用耳机和外放分别测试用户插话、危险事件抢占、TTS 回声及回声中新增用户内容。
9. [ ] 补齐真实 JoyAI-VL 决策适配器和实际图像快照输入后，再验收统一多模态决策，
   不能用 `OMNI_DECISION_MODE=fake` 的结果替代真实模型结果。
10. [ ] 最后重新执行完整 30 分钟实时稳定性测试，记录丢包、乱序、ASR/检测器错误、
    timeline 事件数、高优先级触发、内存、上下文、延迟和 GPU 峰值显存。

真实验收需要分别保留普通场景、纯视觉危险、纯音频危险、音视频一致、音视频冲突、
用户改口、用户插话、危险抢占、纯 TTS 回声、回声中真实插话和重叠说话的原始日志。
只有上述联合场景和完整稳定性测试通过后，才能把里程碑 A 标记为真实设备/GPU 完整验收。
...
4. [√] 增加一个小型环境声音检测器。
5. [√] 在 WebUI Omni 模块建立 timeline，统一音频、视频和输出时间轴。
6. [√] 建立 orchestrator，默认每 1 秒进行一次自主决策。
7. [√] 将音频事件和 ASR partial 注入统一快照决策上下文。
8. [ ] 给 TTS 增加 cancel，完成用户插话测试。
9. [√] 完成 TTS 文本参考回声抑制；物理声学效果留待里程碑 A 设备验收。
