...
from training.omni.midasheng.official_projector import (
    build_frozen_projector_adapter,
    build_official_projector_adapter,
    subsampled_token_count,
)
...
def test_stage_two_unfreezes_only_the_complete_official_projector_before_model_freezing():
    source = torch.nn.Module()
    source.k = 5
    source.net = torch.nn.Sequential(torch.nn.Linear(6400, 3584), torch.nn.GELU(), torch.nn.Linear(3584, 3584))
    projector = build_official_projector_adapter(source, freeze_official=False)
    assert all(parameter.requires_grad for parameter in projector.official_projector.parameters())
    assert all(parameter.requires_grad for parameter in projector.joyai_adapter.parameters())
